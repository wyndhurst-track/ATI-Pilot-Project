...
def read_image_bgr(path):
    ...
    """
    # We don't use cv2.imread, since it gives no feedback on errors while reading the image.
...
def apply_transform(matrix, image, params):
    ...
    #ax = matrix[:2, :].copy()
    ax = matrix[:2, :]
    if ax[0, 1] != 0: #change shape of iamge
        tans_margin = abs(image.shape[1] - image.shape[0]) / 2
        ax[0, 2] = ax[0, 2] - tans_margin
        ax[1, 2] = ax[1, 2] + tans_margin
        output_size_a = image.shape[0]
        output_size_b = image.shape[1]
    else:
        ax = matrix[:2, :]
        output_size_a = image.shape[1]
        output_size_b = image.shape[0]
    output = cv2.warpAffine(
        image,
        ax,
        dsize       = (output_size_a, output_size_b),
...
def resize_image(img, min_side=800, max_side=1333):
    """ return Resized image and the scale for boxes.
    ...
    # resize the image with the computed scale fx is ratio
