...
"""
Python implementation of KCF: http://www.robots.ox.ac.uk/~joao/circulant/
from: https://github.com/uoip/KCFpy
Potentially quicker C++ wrapper: https://github.com/uoip/KCFcpp-py-wrapper
"""
...
class KCFTracker:
	...
	def initialise(self, image, roi):
		self._roi = map(float, roi)
		assert(roi[2]>0 and roi[3]>0)
		self._tmpl = self.__getFeatures(image, 1)
		self._prob = self.__createGaussianPeak(self.size_patch[0], self.size_patch[1])
		self._alphaf = np.zeros((self.size_patch[0], self.size_patch[1], 2), np.float32)
		self.__train(self._tmpl, 1.0)
	...
	def update(self, image):
		...
		loc, peak_value = self.__detect(self._tmpl, self.__getFeatures(image, 0, 1.0))

		if(self.scale_step != 1):
			# Test at a smaller _scale
			new_loc1, new_peak_value1 = self.__detect(self._tmpl, self.__getFeatures(image, 0, 1.0/self.scale_step))
			# Test at a bigger _scale
			new_loc2, new_peak_value2 = self.__detect(self._tmpl, self.__getFeatures(image, 0, self.scale_step))
		...
		x = self.__getFeatures(image, 0, 1.0)
		self.__train(x, self.interp_factor)
	...
	def __subPixelPeak(self, left, center, right):
		divisor = 2*center - right - left   #float
		return (0 if abs(divisor)<1e-3 else 0.5*(right-left)/divisor)

	def __createHanningMats(self):
	...
	def __createGaussianPeak(self, sizey, sizex):
	...
	def __gaussianCorrelation(self, x1, x2):
		if(self._hogfeatures):
	...
	def __getFeatures(self, image, inithann, scale_adjust=1.0):
		...
		if(inithann):
			self.__createHanningMats()  # createHanningMats need size_patch

		FeaturesMap = self.hann * FeaturesMap
		return FeaturesMap

	def __detect(self, z, x):
		k = self.__gaussianCorrelation(x, z)
		...
		if(pi[0]>0 and pi[0]<res.shape[1]-1):
			p[0] += self.__subPixelPeak(res[pi[1],pi[0]-1], pv, res[pi[1],pi[0]+1])
		if(pi[1]>0 and pi[1]<res.shape[0]-1):
			p[1] += self.__subPixelPeak(res[pi[1]-1,pi[0]], pv, res[pi[1]+1,pi[0]])
	...
	def __train(self, x, train_interp_factor):
		k = self.__gaussianCorrelation(x, x)
