...
def test(args):
	...
	t_loader = OpenSetCows2019(args.unknown_ratio, args.repeat_num, suppress_info=False, transform=True, split=args.split, combine=combine, known=known)
	...
	model = embeddings(pretrained=True, num_classes=n_classes, ckpt_path=args.ckpt_path, embedding_size=args.embedding_size, softmax_enabled=args.softmax_enabled)
	...
	for i, (images, _, _, labels, _) in enumerate(trainloader):
...
if __name__ == '__main__':
	...
	parser.add_argument('--split', nargs='?', type=str, default='valid', 
						help='Dataset split to use [\'train, valid, test\']')

	parser.add_argument('--softmax_enabled', type=int, default=0)
	parser.add_argument('--unknown_ratio', type=float,
						help="LEAVE: unknown ratio")
	parser.add_argument('--repeat_num', type=int,
						help="LEAVE: The current repeat")
