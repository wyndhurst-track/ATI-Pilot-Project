...
def plotErrorOrigin():
	base_dir = "D:\\Work\\results\\SoftmaxRTL"
	splits_dir = "D:\\Work\\ATI-Pilot-Project\\src\\Datasets\\data\\OpenSetCows2019"
	...
	for folder in folders:
		...
		x_labels.append(folder_name)

		# Create the path through the first fold
		npzfile_train = np.load(os.path.join(folder, "fold_0/train_triplet_cnn_open_cows_temp_x1_full.npz"))
		npzfile_test = np.load(os.path.join(folder, "fold_0/test_triplet_cnn_open_cows_temp_x1_full.npz"))
		...
		with open(os.path.join(splits_dir, f"{folder_name}.pkl"), 'rb') as handle:
			splits_dict = pickle.load(handle)
		unknown_labels = [int(x) for x in splits_dict[0]['unknown']]
...
def plotTrainingAccuracy():
	...
	# base_dir = "D:\\Work\\results\\SoftmaxRTL\\50-50"
