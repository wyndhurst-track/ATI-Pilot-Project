...
#PBS -N tvt_RGB
### Output files
#PBS -o tvt_RGB_console_output.stdout
#PBS -e tvt_RGB_error_output.stderr
...
OUT_PATH="/work/ca0513/models/RGBDCows2020/LatentSpace/train_valid_test/RGB"
OUT_PATH_NO_DIFFICULT="/work/ca0513/models/RGBDCows2020/LatentSpace/train_valid_test/RGB-no-difficult"

# How many folds to run for (just a single one in this case)
NUM_FOLDS=1

# Train with difficult animals included
python train.py --out_path=$OUT_PATH --img_type="RGB" --num_folds=$NUM_FOLDS

# Train with difficult animals removed
python train.py --out_path=$OUT_PATH_NO_DIFFICULT --img_type="RGB" --num_folds=$NUM_FOLDS --exclude_difficult=1
